...
(=檔案對應=)
    ...
    dockersetup_dumb.sh         [=>]帳號對應腳本(模擬環境使用)

    dockertools_build.sh        [=>]建立清除映像
    dockertools_dpkg.sh         [=>]映像套件資訊

==================================================
(=適用情景=)
[18]  Android(mtk/qcom)   Yocto(mtk/qcom)     Ubuntu(mtk) echo 10 > debian/compat
[22]  Android(mtk/qcom)   Yocto(mtk)          Ubuntu(mtk)
